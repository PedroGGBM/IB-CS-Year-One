...
def random_tour(cities):
    ...
    for letter in range(65, 85):
        cities.append(chr(letter))
    shuffle = random.shuffle(cities)
    return cities

def rdm_tour(cities):
    upd_val = cities
    upd_val.insert(0,'X') and cities.append('X')
    return upd_val

def random_tour_length(cities, x):
    total_length = 0
    temp_1 = 0
    for i in cities:
        total_length += x[temp_1][int(ord(i)-64)]
        temp_1 = ord(i)-64
    return(total_length)

# def fitness_results(total_length):

# def main(x, cities):
    # t = 0
    # while t < 10:
    #     random_tour()
    #     print(rdm_tour(cities))
    #     t += 1


print(random_tour())
y = random_tour(cities)
